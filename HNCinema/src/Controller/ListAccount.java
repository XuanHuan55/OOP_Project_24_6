...
public class ListAccount {
    ...
    public static List<AccountModel> list = new ArrayList<>();

    public ListAccount() {
    }

    public static boolean AddNewUser(String username, String password, String phoneNumber,boolean isManager) {
        for (int i = 0; i < list.size(); i++) {
            if (list.get(i).getUsername().equals(username)) {
                return false;
            }
        }
        list.add(new AccountModel(username, password, phoneNumber,isManager));
        saveToFile();
        return true;
    }
